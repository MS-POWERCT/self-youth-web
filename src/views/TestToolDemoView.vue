<template>
  <van-nav-bar title="测试" left-arrow @click-left="$router.go(-1)" />
  <div class="datetime-demo p-20">
    <h2 class="text-20 font-bold mb-20 ">滚轮日期时间选择器测试</h2>

    <!-- 日期时间选择器测试 -->
    <div class="demo-section bg-white radius-8 p-16 mb-16">
      <div class="demo-title text-16  mb-12">实际使用示例 - 习惯记录</div>
      <div class="demo-content flex items-center justify-between flex-wrap gap-12">
        <div class="date-display text-16 ">
          <span>选择的日期时间：</span>
    ...
    <!-- 分钟时长选择器测试 - 内联形式 -->
    <div class="demo-section bg-white radius-8 p-16 mb-16">
      <div class="demo-title text-16  mb-12">分钟时长选择器 - 调研测试（内联形式）</div>
      <div class="time-picker-wrapper mb-16">
        <WheelTimePicker v-model="selectedDuration" />
      </div>
      <div class="result-display text-16  text-center">
        <span>当前选择：</span>
    ...
    <!-- 使用封装的滚轮日期选择器 -->
    <WheelDateTimePicker v-model:show="showPicker" :default-date="new Date()" @confirm="handlePickerConfirm" />

    <!-- 进度条组件测试 -->
    <div class="demo-section bg-white radius-8 p-16">
      <div class="demo-title text-16  mb-12">进度条组件测试</div>
      <ProgressBar :current="18" :total="50" />
    </div>
  </div>
</template>
